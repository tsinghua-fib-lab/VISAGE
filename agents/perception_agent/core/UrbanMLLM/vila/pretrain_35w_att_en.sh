...
bs=6  # Adjust batch size as needed for your single GPU
echo "number of nodes:" $n_node
echo "per device batch size:" $bs
echo "node rank:" $CURRENT_RANK
export CUDA_VISIBLE_DEVICES=0,3,5,7

torchrun --nnodes=$n_node --nproc_per_node=6 --master_port=25001 \
    ...
    --bf16 True \
    --output_dir /data3/zhangxin/wuwen/vila-1.5/last_checkpoint/pretrain_att_35w_clean_textenhance\
    --num_train_epochs 1 \
