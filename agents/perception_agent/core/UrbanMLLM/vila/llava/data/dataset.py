...
class LazyMMC4Dataset(Dataset):
    ...
    def __init__(
        ...
        import pickle
        data_list  = [json.loads(q) for q in open("/data2/zhangxin/data/186w_sv_si_data_wuwen_clean_lm2.jsonl", "r")]
        #data_list  = [json.loads(q) for q in open("/mnt/public/vila-1.5/data/92w_sv_si_data_wuwen.jsonl", "r")]
        ...
        '''
        print("* loaded totally {} samples".format(len(data_list)))
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if not self.text_only:
            for sample_image, sim_vec in zip(info["image_info"], sim_matrix):
                ...
                #image = Image.open(sample_image["image_name"]).resize((336,336)).convert("RGB")
                image = Image.open(sample_image["image_name"]).convert("RGB")
        ...
        #    if ix==1:
        #        continue
        #    sv_sentences.append(sentences[ix])
        #sentences=sv_sentences
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for ix in range(len(input_ids)):
            ...
            new_image_num.append(torch.tensor(num_images, dtype=torch.int8))
        ...
        if len(new_images) > 0:
            batch["images_num"]=torch.stack(new_image_num)
