...
import sys
sys.path.append('/data2/zhangxin/llm/code/vila')
import argparse
import torch
import os
import json
...
def eval_model(args):
    ...
    for (input_ids, image_tensor), line in tqdm(zip(data_loader, json_file), total=len(json_file)):
        ...
        elif image_tensor.ndim == 5:  # batch_size x seq_len x image_channels
            image_tensor = image_tensor.flatten(0, 1)
        with torch.inference_mode():
            image_features = model.encode_images(image_tensor.to(dtype=torch.float16, device='cuda', non_blocking=True), \
                torch.tensor([image_tensor.shape[0]], device='cuda'))
            image_features = image_features.mean(dim=1)
            image_features = image_features.mean(dim=0)
        feature_vector[idx] = image_features.cpu().detach().numpy()
    year = args.question_file.split('/')[-1].split('.')[0].split('_')[1]
    import pickle
    
    with open("/data2/zhangxin/sy_results/image_mlp_feature/{}.pkl".format(year),'wb') as file:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_new_tokens", type=int, default=6600)
