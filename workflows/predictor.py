...
# ============================================================
# predictor.py
# Standalone Regression + Visualization Module (integrated)
# ============================================================
"""
Run K-fold regression (SVR & KNN) on tract-level embeddings produced
by the VISAGE pipeline, aggregate fold results, visualize combined
predictions, and save metrics.

Usage (module):
    from predictor import run_kfold_regression
    run_kfold_regression(base_data_dir=..., num_folds=5, output_dir=..., segregation_jsonl_paths=[...])

Usage (CLI):
    python predictor.py \
        --base-data-dir /data3/maruolong/VISAGE/data/processed/training_data/31_cities \
        --num-folds 5 \
        --output-dir /data3/maruolong/VISAGE/data/processed/training_data/31_cities/regression_viz \
        --seg-paths /path/to/city1.jsonl,/path/to/city2.jsonl
"""
...
def evaluate_model(name: str, Y_true: np.ndarray, Y_pred: np.ndarray) -> dict:
    """Compute R2, MAE, MSE, and RMSE for given true and predicted values."""
    if len(Y_true) > 0:
        mse = mean_squared_error(Y_true, Y_pred)
        rmse = np.sqrt(mse)
        return {
            f"{name} R2": float(r2_score(Y_true, Y_pred)),
            f"{name} MAE": float(mean_absolute_error(Y_true, Y_pred)),
            f"{name} MSE": float(mse),
            f"{name} RMSE": float(rmse)  # <--- Added RMSE here
        }
    else:
        return {
            f"{name} R2": float("nan"),
            f"{name} MAE": float("nan"),
            f"{name} MSE": float("nan"),
            f"{name} RMSE": float("nan")
        }
...
def run_kfold_regression(
    ...
    num_folds: int = 5,
    output_dir: str = "/data3/maruolong/VISAGE/data/processed/training_data/31_cities/regression_viz",
    ...
    for fold in range(1, num_folds + 1):
        ...
        svr_grid = GridSearchCV(
            SVR(kernel='rbf'), svr_params, cv=5, scoring='r2', n_jobs=-1, verbose=0
        )
        svr_grid.fit(X_train_pca, Y_train)
        svr_best = svr_grid.best_estimator_
        svr_pred = svr_best.predict(X_test_pca)
        
        # Calculate metrics (now includes RMSE)
        svr_metrics = {
        ...
        if verbose:
            # Print RMSE for immediate feedback
            print(f"   SVR Test R2: {svr_metrics.get('Test SVR R2', 0):.4f}, RMSE: {svr_metrics.get('Test SVR RMSE', 0):.4f}")
        ...
        knn_grid = GridSearchCV(
            KNeighborsRegressor(), knn_params, cv=5, scoring='r2', n_jobs=-1, verbose=0
        )
    ...
    if combined_true and combined_pred:
        ...
        plot_density_scatter(combined_true, combined_pred, "SVR 5-Fold Segregation Prediction", density_path)
        if verbose:
...
def _parse_args():
    ...
    parser.add_argument("--base-data-dir", type=str, default="/data3/maruolong/VISAGE/data/processed/training_data/31_cities",
    ...
    parser.add_argument("--output-dir", type=str, default="/data3/maruolong/VISAGE/data/processed/training_data/31_cities/regression_viz",
