...
from experiment_tracker import ExperimentTracker, calculate_aggregated_metric
...
def run_visage_pipeline(
    ...
    custom_viz_output_dir: Optional[str] = None  # <--- Path to 'runs/.../artifacts'
):
    """
    ...
    if run_regression_viz:
        ...
        if run_kfold_regression is not None:
            ...
            # --- START: Experiment Tracking Logic ---
            if tracker and experiment_id:
                print("📝 Calculating aggregated metrics for Tracker...")
                # Initialize summary dictionary
                summary_metrics = {}
                
                # Define all metrics we want to average and log
                # Format: (List Key in metrics_viz, Metric Name Key in per-fold dict)
                targets = [
                    ('SVR_per_fold', 'Test SVR R2'),
                    ('SVR_per_fold', 'Test SVR RMSE'),
                    ('SVR_per_fold', 'Test SVR MAE'),
                    ('SVR_per_fold', 'Test SVR MSE'),
                    ('KNN_per_fold', 'Test KNN R2'),
                    ('KNN_per_fold', 'Test KNN RMSE'),
                    ('KNN_per_fold', 'Test KNN MAE'),
                    ('KNN_per_fold', 'Test KNN MSE')
                ]

                # Loop through and calculate averages
                for list_key, metric_key in targets:
                    fold_data = metrics_viz.get(list_key, [])
                    avg_val = calculate_aggregated_metric(fold_data, metric_key)
                    
                    if avg_val is not None:
                        summary_metrics[metric_key] = avg_val
                
                # Log EVERYTHING to the tracker
                tracker.log_results(
                    experiment_id=experiment_id,
                    detailed_results=metrics_viz, # Stores full fold data
                    summary_metrics=summary_metrics # Stores averaged scores
                )
            # --- END: Experiment Tracking Logic ---
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_folds", type=int, default=5, help="Number of folds for K-fold dataset generation")
    ...
    # --- START: Experiment Tracker Initialization ---
    tracker = ExperimentTracker(root_dir="/data3/maruolong/VISAGE/experiments")
    print("\n==============================")

    # Define initial configuration to track as metadata
    initial_config = {
    ...
    # Start the experiment and get the unique ID
    current_exp_id, current_run_dir = tracker.start_experiment(initial_config)
    # Define Artifacts Directory (For saving plots and prediction files)
    artifacts_dir = os.path.join(current_run_dir, "artifacts")
    # --- END: Experiment Tracker Initialization ---
    ...
    run_visage_pipeline(
        ...
        custom_viz_output_dir=artifacts_dir
    )

    # Optional: Print comparison of all experiments at the end
